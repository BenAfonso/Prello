...
export function addCollaborator (dispatch, boardId, title) {
  addCollaboratorDistant(boardId, title).then((collaborator) => {
      // <= HANDLED FROM SOCKETS
  }).catch(err => {
    return err
  })
...
export function addCollaboratorLocal (board) {
  if (board) {
    store.dispatch({
      type: 'ADD_BOARD',
      payload: board
