...
module.exports = function (router, controller) {
  /**
  * @swagger
  * /analytics/boards/{boardId}/cards:
  *   get:
  *     tags:
  *       - Analytics
  ...
  */
  router.get('/analytics/boards/:boardId/cards', [requiresLogin], function (req, res) {
    if (req.query.provider === 'ThePrello') {
      if (req.user.theprello !== undefined) {
        request({
          ...
          method: 'get',
          url: `https://theprello-api.igpolytech.fr/api/analytics/boards/${req.params.boardId}/cards`
        }, (error, response) => {
      ...
      controller.getCardsAnalytics(req.params.boardId, req.query.per, req.query.from, req.query.to).then((data) => {
